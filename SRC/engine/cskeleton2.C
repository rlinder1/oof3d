...
//This function is theoretically equivalent to checking illegality and then
//calling illegal(), but there have been problems getting changes to member
//variables to "stick". If that issue is resolved, delete this function and
//replace its use with checkIllegality() followed by illegal()
bool CSkeleton::checkIllegalityBool() {
  for(unsigned int i=0; i<nelements(); ++i) {
    if(elements[i]->illegal()) {
      //The following line might not have an effect due to issues with this class
      illegal_ = true;
      return true;
    }
  }
...
double CSkeletonBase::getUnweightedHomogeneity() const {
  ...
  if(unweighted_homogeneity_computation_time < getTimeStamp()) {
    try {calculateUnweightedHomogeneity();}
    catch (...) {
      throw ErrHomogeneityNotCalculable();
    }
  }
...
void CSkeletonBase::calculateUnweightedHomogeneity() const {
  ...
  DefiniteProgress *progress = 
    dynamic_cast<DefiniteProgress*>(getProgress("Calculating unweighted homogeneity",
						DEFINITE));
  const CMicrostructure *ms = getMicrostructure();
  try {
    for (CSkeletonElementIterator elit = beginElements();
	...
	if(!(*elit)->illegal()) {
	  unweightedHomogIndex += (*elit)->homogeneity(ms);
...
//on the alpha level used in skeleton modifiers
//Alpha = 0 gives only shape energy, while alpha = 1 gives only homogeneity energy
...
double CSkeletonBase::getWeightedShapeEnergy() const {
  ...
  if (weighted_shape_energy_computation_time < getTimeStamp()) {
	...
	weightedShapeEnergy +=
	  (*elit)->energyTotal(getMicrostructure(), 0) * (*elit)->volume();
	vtot += (*elit)->volume();
      }
